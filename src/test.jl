...
function refractive_index(gas)
    if gas == :O3
        data = readdlm(raw"C:\Users\ms246\Dropbox (Heriot-Watt University Team)\RES_EPS_Lupo\Projects\Mohammed\phd\ozone absorption spectrum\ozone_refractive_index.txt")
    end
    x = data[:,1]*1e-6
    y = data[:,2]
    y
...
function O3_func(W,p,Tk,a)
    # β   = W./c .+ 0.5.*W.*p.*Tk_0.*refractive_index(:O3)./(c.*p_0.*Tk) .- .5*u^2*c/(a^2*W)
    β   = W/c .+ 0.5.*W.*p.*Tk_0.*refractive_index(:O3)./(c.*p_0.*Tk) .- .5*u^2*c./(a^2.0.*W)

    dW = W[2]-W[1]
    β1 = diff(β, dims=1)/dW
    β2 = diff(β1, dims=1)/dW
    return cat(β2, [0,0], dims=1)
end


data = O3_func(W,1,300,13e-6)
λ = 2pi*c./W
plot(λ[100e-9.<λ.<2e-6]*1e9,data[100e-9.<λ.<2e-6]*1e27,
            xlabel="wavelength (nm)",ylabel="dispersion (\$ps^2/Km\$)", title="dispersion")





# function sellmeier(λ, Bs, Cs, p, Tk)
#     Tk_0 = 272.15
#     p_0 = 1
#     terms = 0
#     for i in 1:length(Bs)
#         terms += p*Tk_0*Bs[i]*λ^2/(λ^2-Cs[i])/(p_0*Tk)
#     end
#     return sqrt(terms+1)
# end
# # argon
# Bs = [20332.29*10^-8.0, 34458.31*10^-8.0]
# Cs = [206.12*10^-6.0*10^-12.0, 8.066*10^-3.0*10^-12.0]
# λ = Vector(100e-9:1e-9:1e-6)
# f(λ) = sellmeier(λ, Bs, Cs, 1, 300)
# # plot(λ, f.(λ))
#
# r(λ) = rindex.(λ, :Ar, 1, 300; dif=2)
# plot(λ, r.(λ))
